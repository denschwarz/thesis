...
\subsubsection{XCone algorithm}
	XCone is an exclusive jet algorithm, returning conical jets. It is well suited for analysis where the final state and therefore the expected number of jets is known since it returns a fixed number of jets. Thus, a physical final state has direct influence on the jet finding. \\
...
\subsection{$\cancel{E}_T$ and $S_T$}
	...
	In this analysis additionally a definition for $S_T$ is used which just takes leptonic activity into account. It is referred to as $S_T^{\text{lep}}$ (see Eq. \ref{STlep}).
