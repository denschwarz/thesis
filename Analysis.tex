...
\subsection{Closure Tests}
	...
	\begin{figure}[tb]
		...
		\caption{A unfolding with a MC sample split into $10\%$ pseudo data and $90\%$ to fill the migration matrix is performed four times with different events selected to be pseudo data every time. Every unfolding output is compared with a particle level distribution of the $90\%$ Monte Carlo whose uncertainty is represented by the line thickness. Fluctuations around the truth distribution is observed, probably arising from the simple migration matrix.}
	...
	\begin{figure}[tb]
		...
		\end{subfigure}		
		\caption{Unfolding with statistically independent MC samples where the renormalisation and factorisation scale in the pseodo data set is scalled up (a) and down (b). Is is compared with its truth distribution and a truth distribution of the sample used to fill the migration matrix.}
...
\label{sec:results}
	...
	\begin{figure}[tb]
		...
		\label{fig:Result2}
		\end{subfigure}		
		\caption{As a result of this analysis, the unfolded data distribution (a) and the reconstructed jet mass (b) are shown.}
	\end{figure}
